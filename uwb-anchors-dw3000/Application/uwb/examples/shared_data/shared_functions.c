...
uint16_t mywaitforsysstatus(uwb_t *uwb,uint32_t *lo_result, uint32_t *hi_result, uint32_t lo_mask, uint32_t hi_mask,waitsys_t waitsys){
    ...
    case FIRSTWAITSYS:
    {
        ...
        if (lo_mask){
        	if(!((lo_result_tmp = dwt_readsysstatuslo()) & (lo_mask))){
        		...
        		uwb->waitresult = WAITSYS_NOT_PASSED;
        	}
        	else{
        		    uwb->waitresult = PASS_FIRST_WAITSYS;
        	}
        }
        else if (hi_mask){
    	        ...
    	        if (lo_mask){
    	        	if(!((lo_result_tmp = dwt_readsysstatuslo()) & (lo_mask))){
    	        		...
    	        		if (hi_mask){
    	        		    ...
    	        		    uwb->waitresult = PASS_SECOND_WAITSYS;
    	        	}
    	        }
    	        else if (hi_mask){
